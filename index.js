const url =
  "https://cors-anywhere.herokuapp.com/https://datos.madrid.es/egob/catalogo/216619-0-wifi-municipal.json";
...
function filtro(resultado) {
  ...
  let filtrado = resultado.filter((e) => {
    if (
      e.address["street-address"].toLowerCase().includes(input.toLowerCase()) ||
      e.address["postal-code"].toLowerCase().includes(input.toLowerCase())
    ) {
      return true;
    }
  });
...
function mensajeError() {
  ...
  error.innerText =
    "No hay ningún lugar con wifi gratis en esta dirección. Por favor, busque otra vez.";
